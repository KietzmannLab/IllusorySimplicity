...
# load weights for dvd-s
weights_path = "/share/klab/lzejin/lzejin/codebase/P001_DVD/logs/shared_weights/resnet50_mpe1.0_alpha0.1_dn150.0_imagenet256_0.0001_dev_evd_b1c1cs1_T_normal_seed_1/weights/checkpoint_best.pth"

state_dict = torch.load(weights_path, map_location='cpu')['state_dict']

# remove module. from keys to be compatible with single gpu model
state_dict = {
    k[7:]: v for k,v in state_dict.items()
}

# load model and find layers that match selection criteria
model = timm.create_model('resnet50', pretrained=False, exportable=True)
