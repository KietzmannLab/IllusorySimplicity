...
#SBATCH --output=./logs/interarea_N_v1_12_%A_%a.out
#SBATCH --error=./logs/interarea_N_v1_12_%A_%a.err
#SBATCH --array 0-150
...
python -u scripts/allMUA_inter_area_fit.py --run-name $run_name --target-times $start $end $stride --delays $stride 81 $stride --source-rois 1 --target-rois 2 --monkey $monkey --session-ids $sessions --session-ids-for-channel-selection $sessions_for_dprime --dataset allMUA --neural-data $neuraldata --source-arrays $source_arrays --target-arrays $target_array --trial-averaged 0 --include-pairwise-interaction-features $interaction_features
